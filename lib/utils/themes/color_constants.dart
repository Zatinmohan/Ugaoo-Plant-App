import 'package:flutter/material.dart';

class ColorConstants {
  static const Color kBackgroundColor = Color(0xffDFF2EA);
  static const Color kPrimaryAccentColor = Color(0xff567600);
  static const Color kSecondaryAccentColor = Color(0xffA2D4BF);
  static const Color kPrimaryTextColor = Color(0xff000000);
  static const Color kDisabledButtonColor = Color(0xffD3D3D3);
  static const Color kErrorButtonColor = Color(0xffd19191);
  static const Color kSecondaryTextColor = Color(0xffFFFFFF);
  static const Color kPrimaryIconColor = Color(0xff000000);
  static const Color kPrimaryAppBarColor = Color(0xffDFF2EA);
  static const Color kSecondaryTextColor_2 = Color(0xffA9A9A9);
}
